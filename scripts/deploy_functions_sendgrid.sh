...
# Deploy Cloud Functions with SendGrid configuration
# Reads SendGrid credentials from functions/.env and deploys with environment variables
...
info "Deploying Cloud Functions with SendGrid"
...
if [ ! -f "functions/.env" ]; then
    ...
    info "Run: ./scripts/setup_sendgrid.sh first"
    exit 1
fi

# Load environment variables from .env
info "Loading SendGrid configuration..."
export $(cat functions/.env | grep -v '^#' | xargs)

if [ -z "$SENDGRID_API_KEY" ] || [ -z "$SENDGRID_SENDER_EMAIL" ]; then
    error "Missing SendGrid configuration in functions/.env"
    info "Required variables: SENDGRID_API_KEY, SENDGRID_SENDER_EMAIL"
    exit 1
fi

success "SendGrid configuration loaded"
info "Sender: $SENDGRID_SENDER_EMAIL"
echo ""
...
if [ ! -d "node_modules" ] || [ ! -d "node_modules/@sendgrid" ]; then
...
gcloud functions deploy cleanupDeletedUsers \
  ...
  --set-env-vars "SENDGRID_API_KEY=$SENDGRID_API_KEY,SENDGRID_SENDER_EMAIL=$SENDGRID_SENDER_EMAIL,SENDGRID_SENDER_NAME=${SENDGRID_SENDER_NAME:-Artist Finance Manager}" \
  --project=$PROJECT_ID
...
info "  Provider: SendGrid"
info "  Sender: $SENDGRID_SENDER_EMAIL"
info "  Free tier: 12,000 emails/month"
info ""
info "Test email sending:"
info "  You can test when a user registers or logs in"
info ""
info "Monitor emails:"
info "  https://app.sendgrid.com/email_activity"
info "======================================"
